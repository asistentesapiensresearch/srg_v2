    ...
    <div className={`${containerBaseStyles} ${className}`}>
      ...
      <div className="relative">
        {icon && iconPosition === 'left' && (
          <div className={`absolute left-3 top-1/2 transform -translate-y-1/2 text-[var(--color-text-tertiary)] ${iconStyles[size]} pointer-events-none`}>
            {icon}
          </div>
        )}
        ...
        <div className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-[var(--color-text-tertiary)] pointer-events-none ${iconStyles[size]}`}>
          <ChevronDown />
        </div>

        {icon && iconPosition === 'right' && (
          <div className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-[var(--color-text-tertiary)] ${iconStyles[size]} pointer-events-none`}>
            {icon}
          </div>
        )}
      </div>
      ...
      {helperText && !hasError && (
        <p id={`${id}-helper`} className="mt-2 text-sm text-[var(--color-text-caption)]">
          {helperText}
        </p>
      )}

      {hasError && (
        <p id={`${id}-error`} className="mt-2 text-sm text-[var(--color-danger)] flex items-center gap-1" role="alert">
