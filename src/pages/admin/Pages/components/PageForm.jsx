...
export const PageForm = ({ open, onClose, onSave, page }) => {
    const TEMP_FOLDER = "pages/temp/";
    ...
    useEffect(() => {
        if (open) {
            if (page) {
                ...
                if (page.coverImage) {
                    getUrl({ path: page.coverImage }).then((res) => {
    ...
    const handleInternalSave = async () => {
        ...
        try {
            ...
            if (finalKey.startsWith(TEMP_FOLDER)) {
                finalKey = await moveIconToDefinitiveFolder("pages/", finalKey, formData.slug);
            }

            // Si estamos editando y la imagen cambió, borrar la vieja
            if (page?.id && page.coverImage && page.coverImage !== finalKey) {
                await remove({ path: page.coverImage });
            }

            const finalData = { ...formData, coverImage: finalKey };
            await onSave(finalData, page?.id);
            onClose();
        } catch (error) {
    ...
    return (
        <Dialog open={open} onClose={handleCancel} fullWidth maxWidth="sm">
            <DialogTitle>{page ? 'Editar Página' : 'Nuevo Página'}</DialogTitle>
            <DialogContent>
